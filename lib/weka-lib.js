            ...
            function(callback) {
                ...
                async.eachSeries(arffObj.data, function(obj, dataCb) {
                    ...
                    async.eachSeries(_.keys(obj), function(key, mapCb) {
                        //if (arffObj.types[key]) {
                        if (!arffObj.types[key]) {
                            console.log(key);
                            console.table(arffObj.types)
                        }
    ...
    exports.correlationMatrix = function(data, options, cb) {
        ...
        async.waterfall([
            ...
            function(fileIdTraining, callback) {
                ...
                child = exec('java -cp ../bin/weka.jar ' + options.evaluator + ' ' +
                    options.e_params +
                    ' -s "' + options.ranker + '"' +
                    ' -i ' + fileIdTraining,
    ...
    exports.classify = function(data, test, options, cb) {
        ...
        async.waterfall([
            ...
            function(fileIdTest, callback) {
                ...
                child = exec('java -cp ../bin/weka.jar ' + options.classifier + ' ' +
                    options.params +
                    ' -t ' + fileIdTraining +
                    ' -T ' + fileIdTest +
                    ' -no-cv -v -p 0',
    ...
    exports.cluster = function(data, options, cb) {
        ...
        async.waterfall([
            ...
            function(fileIdTraining, callback) {
                ...
                child = exec('java -cp ../bin/weka.jar ' + options.clusterer + ' ' +
                    options.params +
                    ' -t ' + fileIdTraining +
                    ' -p 0',
